...
class Elem:
    ...
    class ValidationError(Exception):
        ...
        def __init__(self):
            super().__init__("Invalid content for Elem class")
    ...
    def __str__(self):
        ...
        if self.tag_type == "double":
            result = (
                f"<{self.tag}{self.__make_attr()}>{self.__make_content()}</{self.tag}>"
            )
...
if __name__ == "__main__":
    e = Elem(
        ...
        content=[
            Elem(
                tag="head", content=Elem(tag="title", content=Text('"Hello ground!"'))
            ),
            Elem(
                tag="body",
                content=[
